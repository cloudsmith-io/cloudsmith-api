...
> PackageBulkActionResponse recycle_bin_recycle_bin_action(owner, data=data)
...
try:
    api_response = api_instance.recycle_bin_recycle_bin_action(owner, data=data)
    pprint(api_response)
except ApiException as e:
    print("Exception when calling RecycleBinApi->recycle_bin_recycle_bin_action: %s\n" % e)
```
